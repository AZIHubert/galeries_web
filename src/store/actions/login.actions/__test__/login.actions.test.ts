...
describe('confirmation', () => {
  describe('action', () => {
    ...
    it('should fetch login success', () => {
      ...
      expect(actions[0].type).toEqual(`${LOGIN} Fetch`);
      expect(actions[1].payload).toEqual({ data: { status: 'pending' } });
      expect(actions[1].type).toEqual(`${LOGIN} Set`);
      expect(actions[2].type).toEqual(`${LOGIN} ${API_REQUEST}`);
      expect(actions[3].payload).toEqual({ data: true });
      expect(actions[3].type).toEqual(LOADER_SET);
      expect(actions[4].type).toEqual(`${LOGIN} ${API_SUCCESS}`);
      expect(actions[5].payload).toEqual({ data: { status: 'success' } });
      expect(actions[5].type).toEqual(`${LOGIN} Set`);
      expect(actions[6].type).toEqual(`${USER} Fetch`);
      expect(actions[7].type).toEqual(`${USER} ${API_REQUEST}`);
      expect(actions[8].payload).toEqual({ data: true });
      expect(actions[8].type).toEqual(LOADER_SET);
      expect(actions[9].type).toEqual(`${USER} ${API_SUCCESS}`);
      expect(actions[10].type).toEqual(`${USER} Set`);
      expect(actions[11].payload).toEqual({ data: false });
      expect(actions[11].type).toEqual(LOADER_SET);
    });
    it('should fetch login error with field error', () => {
      ...
      expect(actions[5].type).toEqual(`${LOGIN} Set`);
      expect(actions[6].payload).toEqual({ data: false });
    ...
    it('should fetch login error with global error', () => {
      ...
      expect(actions[7].payload).toEqual({ data: false });
